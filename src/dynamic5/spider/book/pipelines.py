...
from spider.book.items import BookItem, CommentItem, PersonItem
from app.models import Book, Person, Comment
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, BookItem):
            ...
            for author_id in author_ids:
                person, created = Person.objects.get_or_create(id=author_id)
                logger.info('Created person %s' if created else 'Person %s exists', person)
                persons.append(person)
            item.instance.authors.set(persons)
        
        if isinstance(item, PersonItem):
            item.instance.save()
            logger.info('Saved person %s', item.instance)
